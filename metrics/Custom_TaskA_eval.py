...
def evaluate_normal_questions(df_run, df_qrels):
    ...
    The evaluation is performed using pytrec_eval tool (common tool for evaluating IR systems in python)
    """
    try:
        # Ensure that columns are strings, as pytrec_eval requires string-based identifiers.
        df_run.loc[:, "qid"] = df_run["qid"].astype(str)
        df_run.loc[:, "docid"] = df_run["docid"].astype(str)
        df_qrels.loc[:, "qid"] = df_qrels["qid"].astype(str)
        df_qrels.loc[:, "docid"] = df_qrels["docid"].astype(str)

        # Drop unnecessary columns
        df_qrels = df_qrels.drop(columns=["Q0"], errors="ignore")
        df_run = df_run.drop(columns=["Q0"], errors="ignore")

        # Convert score and relevance columns to numeric types
        df_run.loc[:, "score"] = pd.to_numeric(df_run["score"], errors="coerce")
        df_qrels.loc[:, "relevance"] = pd.to_numeric(df_qrels["relevance"], errors="coerce")

        # Drop any rows with NaN values after conversion
        df_run.dropna(subset=["qid", "docid", "score"], inplace=True)
        df_qrels.dropna(subset=["qid", "docid", "relevance"], inplace=True)

        # Convert dataframes to dictionaries compatible with pytrec_eval
        qrels_dict = convert_to_dict(df_qrels, column1="qid", column2="docid", column3="relevance")
        run_dict = convert_to_dict(df_run, column1="qid", column2="docid", column3="score")
        ...
        print(f"KeyError: {e}. Please ensure the dataframe contains the required columns.")
...
def get_optimal_map(query_to_doc_ground, query_to_doc_preds):
    # Print the contents of query_to_doc_ground and query_to_doc_preds
    ...
    # Print the processed query_to_docs to check if '114' exists
